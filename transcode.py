...
class MinicastTranscoder:
    ...
    def transcode_gif(self, input_path: str, output_path: str, 
        ...
        # Add loop if requested (only for input, not output)
        if config['loop']:
            cmd.insert(2, '-stream_loop')  # Insert after ffmpeg
            cmd.insert(3, '-1')            # Insert loop value
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Minicast GIF Transcoder - Convert GIFs to low-bandwidth H.264 streams"
    )
    parser.add_argument('input', help='Input GIF file path')
    parser.add_argument('output', help='Output MP4 file path')
    parser.add_argument('--resolution', default='160x120', help='Output resolution (default: 160x120)')
    parser.add_argument('--fps', type=int, default=2, help='Output framerate (default: 2)')
    parser.add_argument('--bitrate', default='50k', help='Target bitrate (default: 50k)')
    parser.add_argument('--crf', type=int, default=28, help='Constant Rate Factor (default: 28)')
    parser.add_argument('--stream-ready', action='store_true', 
                       help='Create stream-ready file with optimized settings')
    ...
    if args.stream_ready:
        # Create stream-ready file
        output_path = transcoder.create_stream_ready_file(args.input)
        if output_path:
            logger.info(f"✅ Stream-ready file created: {output_path}")
        else:
            logger.error("❌ Failed to create stream-ready file")
        ...
        if transcoder.transcode_gif(args.input, args.output, custom_config):
            logger.info(f"✅ Transcoding successful: {args.output}")
        else:
